...
We answer this question with \system{}, a layered monitoring design and a controlled sensor-selection methodology. Our prototype testbed contains six RTX~3090 GPUs, three oscilloscopes, a thermal camera, a visible-light camera, a passive fiber network tap, and contact temperature sensors. The scopes provide high-rate electrical observations that emulate a future rail-monitor ADC; the cameras and contact sensors expose slower spatial and thermal dynamics; and the network tap captures content-free packet timing and size metadata. NVML remains the low-cost always-on layer, while \wave{} is reproduced as an offline, high-information reference rather than misrepresented as a deployable monitor.
...
\system{} (this work) & NVML plus electrical, thermal, optical, and network evidence; training detection & Tiered; passive modalities are independent & New unseen workload and telemetry attacks & Prototype instruments must be reduced to deployable sensor specifications \\
\bottomrule
\end{tabularx}
\end{table*}
...
\node[sensor,right=0.9cm of gpu,yshift=-0.45cm] (thermal) {Tier 1: thermal/contact\\and equipment ROI};
\node[sensor,right=0.9cm of gpu,yshift=-1.35cm] (net) {Tier 1: fiber tap\\metadata only};
...
All host-side logs use \texttt{CLOCK\_MONOTONIC\_RAW} and record both sample and receipt timestamps. At run boundaries, a scripted load step supplies a common physical marker. Oscilloscope traces are aligned to the power edge; thermal, optical, and contact traces are aligned by cross-correlation with the slower response; network timestamps are aligned to a logged burst generated outside the measured workload. If the scopes support external triggering, one trigger is fanned out to all instruments. Otherwise, alignment error is measured rather than assumed.

Channel-health features include missing-sample fraction, timestamp drift, stuck values, clipping, impossible slopes, camera occlusion, network-link loss, and disagreement between NVML power/temperature and the corresponding external modalities. A missing or unhealthy sensor is a separate state, never silently imputed as benign.
...
Visible camera & Equipment-only fan/LED motion and state & Optical-flow energy, fan periodicity, LED duty cycle, occlusion score & Independent fan/controller response; retained only if incremental \\
Fiber tap & Packet metadata, never payload & Bytes/s, packets/s, directionality, packet-size quantiles, burstiness, inter-arrival PSD, collective-like periodicity, correlation with GPU power & External data/collective phases; limited for intra-node PCIe traffic \\
...
The available server contains six NVIDIA RTX~3090 GPUs. The physical instrumentation includes three oscilloscopes, a thermal camera, a visible-light camera, a fiber network tap, and temperature sensors. The current inventory does not specify CPU, DRAM, motherboard/PCIe topology, OS, driver, CUDA, framework, scope bandwidth, probe type, camera frame rate/resolution, tap speed, or temperature-sensor accuracy. These are experimental variables, not cosmetic details, and are therefore explicitly collected before the first run.

Because the statement ``three oscilloscopes, one channel per GPU'' is ambiguous, the default plan assumes three GPUs can be electrically instrumented simultaneously. If more isolated channels are available, the manifest is updated and all channels are calibrated. The six GPUs are first characterized individually, then run in balanced groups so that GPU identity and sensor assignment are not confounded.

\begin{experimentbox}{Inventory, topology, and safety qualification}
Record: GPU UUID/SKU/VBIOS, PCIe bus and NUMA topology, any NVLink bridges, CPU, DRAM, storage, NIC, PSU, OS/kernel, NVIDIA driver, CUDA, cuDNN, NCCL, PyTorch, Nsight Compute, power/clock limits, ambient conditions, and every sensor model/firmware/rate/resolution. Draw the exact cable/tap/probe topology. Use only manufacturer-rated isolated current probes, shunts, or instrumented power cables. Do not probe mains or exposed GPU rails without a qualified lab procedure. Measure idle noise, clipping, channel gain, timestamp jitter, and GPU-to-channel mapping. Put the resulting table in the paper and archive \texttt{nvidia-smi -q}, \texttt{nvidia-smi topo -m}, and environment-lock outputs.
\end{experimentbox}
...
Telemetry attacks & Freeze, replay, offset, scale, timestamp drift, sample drop, GPU-channel swap, camera occlusion, tap disconnect & Attack duration and affected channels & Unmodified synchronized trace \\
\bottomrule
\end{tabularx}
\end{table*}
...
Every modality is first evaluated alone, then added to the frozen NVML baseline. A sensor is judged on the hard overlap/evasion validation set, not overall clean accuracy. We report feature groups before individual feature rankings to reduce instability. The optical and thermal channels are explicitly allowed to fail; the purpose is to identify a minimum evidence set, not justify every available instrument.
...
Freeze the first fusion detector. Give the evader its feature code and a surrogate training subset. Implement at least five new strategies, including fused-update training, power-shaping/duty-cycle optimization, camera/fan-response mimicry if feasible, network-padding or suppression, and cross-GPU work migration. Hold the entire family out during defender training. Report zero-shot, after-one-round, and leave-one-family-out results at the fixed threshold, with useful-work and energy penalties. Separately inject telemetry replay/spoofing and sensor failures; report direct classification, health-alarm rate, and residual-based detection.
\end{experimentbox}

\section{Hardware Co-Design: From Instruments to a Deployable Monitor}
\label{sec:codesign}

Oscilloscopes and cameras establish observability but are not a production bill of materials. We therefore treat the highest-fidelity synchronized traces as a source and emulate cheaper hardware by causal filtering, downsampling, quantization, clipping, spatial ROI reduction, and duty cycling. The design output is not ``use an oscilloscope''; it is a minimum sample rate, effective number of bits, dynamic range, number and placement of thermal/contact points, network timestamp granularity, and local feature-compute/storage budget.

For electrical sensing, candidate rates are logarithmically spaced from the maximum reliable acquisition rate down to 1~Hz; bit depths include 16, 12, 10, 8, 6, and 4 effective bits. For thermal/visible sensing, we sweep frame rate and replace images with progressively smaller ROI statistics. For the tap, we coarsen timestamps and aggregate packet sizes into bins. For \wave{}, we evaluate counter-family subsets and sampling only selected kernels/windows. All transformations are performed within each training fold to avoid using future samples or test normalization.
...
This section records hypotheses, not results. High-rate electrical sensing is most likely to add information for fused-update and diluted training because weight writes and backward phases create transient structure hidden by 1~Hz averaging. Contact temperature may retain most long-run value of the thermal camera at much lower bandwidth. The thermal camera may nevertheless reveal GPU-to-GPU imbalance and throttling. The fiber tap should help only when relevant traffic crosses the tapped fiber; it cannot observe intra-node PCIe or NVLink and is therefore expected to add little on single-node DDP. Visible imaging is the least certain modality and should be removed unless fan/LED dynamics provide conditional value beyond electrical and thermal features.

These hypotheses cannot be converted into conclusions without the red-box tests. A negative result is useful: ruling out redundant cameras or a tap that misses intra-node collectives narrows the deployable sensor set. The central outcome is the cost-robustness frontier, not the largest possible classifier.
...
The prototype is one six-GPU consumer server, not a datacenter fleet. RTX~3090 results may not transfer to H100/H200/B200 power delivery, HBM, NVLink/NVSwitch, cooling, or confidential-computing modes. Three scopes may instrument only half the GPUs concurrently. A fiber tap cannot see intra-node collectives. Thermal signals depend on ambient conditions, cooling policy, chassis layout, and workload duration. Cameras add physical privacy and deployment concerns. The monitor detects training-like physical behavior, not intent or legal noncompliance. Sensor fusion increases the attack surface and requires a secure evidence path that our software prototype only emulates. Finally, an adaptive adversary may discover a new joint evasion; leave-one-family-out evaluation measures transfer but cannot prove completeness.
...
Multi-GPU training was already easy in NVML corpus & Earlier multi-GPU evasion was detected strongly on datacenter nodes & Network sensing may be redundant unless it exposes a new trust domain & Retain tap only if conditional gain appears; distinguish intra-node from tapped external traffic \\
...
\begin{enumerate}
\item Map each GPU UUID to PCIe slot, power cable, scope channel, camera ROI, contact sensor, and any observable network flow.
