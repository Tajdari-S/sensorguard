...
\begin{abstract}
Hardware-enabled governance requires evidence about accelerator use without inspecting model weights, data, or code. Coarse NVIDIA Management Library (NVML) telemetry is inexpensive, but low-rank and custom-kernel evasions deliberately approach the physical footprint of inference; per-kernel performance counters expose stronger invariants but are too expensive for continuous monitoring. We present \system{}, an architecture that combines always-on NVML with independently measured rail-power evidence, cross-modal tamper residuals, and sequential run-level decisions. Other physical modalities are retained only if they add statistically significant conditional value. On the current 118-run pipeline-validation corpus, a reconstructed NVML detector identifies 22 of 23 training runs in both run-grouped and leave-GPU-out evaluation, with no alerts on 95 non-training runs. However, leave-family-out evaluation identifies 0 of 23 training runs and raises 10 false alerts, showing that device interpolation is not unseen-workload transfer. An RTX~3090 reproduction of \wave{} matches 16/16 tight and 15/16 loose lower-bound cases, but incurs 161\% mean overhead for one NCU metric and 2,750\% for the full metric set. The final evaluation holds out GPUs, days, applications, and the fused-update evasion family, then degrades full-rate electrical traces to derive the minimum trusted ADC specification. \textbf{Final-result fields remaining at freeze:} sensor-fusion gain, time to alert, monitor overhead, and retained sample rate/bit depth.
\end{abstract}
\maketitle
...
We answer this question with \system{}, a layered monitoring design and controlled sensor-selection methodology. The evaluation fleet contains eight RTX~3090 GPUs across three hosts: seven GPUs contribute the current NVML corpus, two verifier GPUs form the electrically instrumented subset, and one 200~W-capped GPU hosts the \wave{} reproduction. Rail power is submission-critical; thermal, acoustic, RF, and network modalities are retained only if they provide conditional value beyond NVML and electrical sensing. NVML remains the low-cost always-on layer, while \wave{} is an offline, high-information reference rather than a deployable monitor.
...
\node[box,minimum width=2.4cm] (gpu) {8-GPU RTX 3090 fleet\\2-GPU sensor subset};
...
The fleet contains eight NVIDIA RTX~3090 GPUs across node2 (five GPUs), verifier (two GPUs), and testbed-node1 (one GPU). Current physical collection is restricted to the two verifier GPUs until probe wiring and safety qualification are complete. The WAVE host is separately power-capped at 200~W, while the remaining cards expose 350--370~W limits. Physical instrumentation includes three two-channel, 10~MHz Pico Technology oscilloscopes; a TOPDON TC001-A thermal camera (256$\times$192 IR, 512$\times$384 TISR, 25~Hz, stated range $-20$--$550^\circ$C); 10K 3950B thermistors; a Dodotronic 200/250~kHz USB ultrasound microphone; a NooElec NESDR SMArt bundle; and a mirrored-Ethernet path.
...
The protocol audit in Figure~\ref{fig:generalization-audit} makes the gap concrete. Leave-GPU-out evaluation also detects 22/23 training runs with no false-positive runs, but leave-family-out evaluation detects 0/23 and raises 10 false alerts among 95 non-training runs (0.873/GPU-hour). The two-day corpus cannot estimate training transfer in both directions: training examples occur only on August~25, so the fold trained on August~24 lacks the classes required by the two-stage detector. We report that fold as non-estimable rather than silently pooling days. Thus the present baseline supports device interpolation on this homogeneous fleet, not unseen-workload or temporal transfer.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{generalization-audit.pdf}
\caption{The reconstructed baseline is sensitive to evaluation protocol. Bars show aggregate run-level outcomes; TPR whiskers are Wilson 95\% intervals. Downward triangles mark one-sided 95\% false-alert-rate upper bounds when zero events were observed. Day holdout is omitted because one direction is not estimable.}
...
\begin{table}[t]
\centering
\caption{Current baseline audits and slots for primary results. Final values must be run-level on the untouched test split.}
\label{tab:primary}
\scriptsize
...
\midrule
NVML run-grouped diagnostic & 22/23 & 0 observed$^*$ & N/A & \tbd \\
NVML leave-GPU-out audit & 22/23 & 0 observed$^*$ & N/A & \tbd \\
NVML leave-family-out audit & 0/23 & 0.873 & N/A & \tbd \\
NVML held-out final test & \tbd & \tbd & \tbd & \tbd \\
...
\bottomrule
\end{tabularx}
\parbox{\columnwidth}{\scriptsize $^*$11.45 negative GPU-hours; one-sided 95\% zero-event upper bound is 0.262 false alerts/GPU-hour. Audit rows are not the untouched final test result.}
\end{table}
...
The NeurIPS work contributes the NVML corpus, adversarial iteration method, and evidence that coarse telemetry detects training~\cite{rahman2026hidden}. The earlier roofline study contributes the observation that non-training workloads overlap training in arithmetic-intensity space~\cite{anonymous2026always}. This paper must contribute new synchronized physical data, sensor and hardware co-design, cross-modal tamper tests, a \wave{} reproduction on Ampere, and a matched overlap evaluation. Reusing earlier data for feature motivation is tracked in the artifact's internal provenance map; headline claims use the new held-out RTX~3090 corpus. If venue policies treat either prior manuscript as concurrent or substantially overlapping, the authors must disclose it and narrow duplicated text/results before submission.

\subsection{Limitations}

The prototype is an eight-GPU consumer fleet with only a two-GPU electrically instrumented subset, not a datacenter deployment. RTX~3090 results may not transfer to H100/H200/B200 power delivery, HBM, NVLink/NVSwitch, cooling, or confidential-computing modes. Additional simultaneous scope channels depend on probe, isolation, triggering, and acquisition-rate constraints. The mirrored Ethernet path cannot see intra-node collectives. Thermal, acoustic, and RF signals depend on ambient conditions, chassis layout, placement, interference, and duration. The monitor detects training-like physical behavior, not intent or legal noncompliance. Sensor fusion increases the attack surface and requires a secure evidence path that our prototype only emulates. Finally, an adaptive adversary may discover a new joint evasion; leave-one-family-out evaluation measures transfer but cannot prove completeness.

\section{Conclusion}

\system{} combines always-on NVML with independent physical evidence, cross-modal tamper checks, and sequential inference. The current artifact establishes a leakage-audited diagnostic baseline, an Ampere WAVE reproduction, calibrated logging, and an empirical roofline toolchain. The final claim is intentionally narrower: the minimum electrical-sensing specification that improves held-out fused-update detection at a fixed false-alert and overhead budget. No modality is claimed useful until it passes the frozen sensor-retention rule on held-out evidence.
